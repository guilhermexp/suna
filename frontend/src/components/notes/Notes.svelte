<script lang="ts">
	...
	import ViewColumns from '../icons/ViewColumns.svelte';
	...
	let previewNote = null; // Nota selecionada para preview
	...
	{#if loaded}
		...
		<div class="flex flex-col gap-1 px-3.5">
			<div class=" flex flex-1 items-center w-full space-x-2">
				...
				<div class="flex items-center gap-1">
					<Tooltip content={$i18n.t('Grid View')}>
						<button
							class="p-1.5 rounded-lg transition {viewMode === 'grid' 
						...
						>
							<ViewColumns className="size-4" />
						</button>
					</Tooltip>
					
					<Tooltip content={$i18n.t('List View')}>
						<button
							class="p-1.5 rounded-lg transition {viewMode === 'list' 
								? 'bg-gray-200 dark:bg-gray-700' 
								: 'hover:bg-gray-100 dark:hover:bg-gray-800'}"
							on:click={() => {
								viewMode = 'list';
		...
		<div class="px-4.5 @container h-full pt-2">
			{#if Object.keys(notes).length > 0}
				<!-- Modo Lista com Preview -->
				{#if viewMode === 'list'}
					<div class="flex gap-4 h-full">
						<!-- Lista de Notas -->
						<div class="w-96 flex-shrink-0 overflow-y-auto">
							{#each Object.keys(notes) as timeRange}
								...
								{#each notes[timeRange] as note, idx (note.id)}
									<div
										class="mb-2 p-3 rounded-lg cursor-pointer transition-all
									...
									>
										<div class="flex items-start justify-between">
											...
											>
												<button
													class="ml-2 p-1 dark:text-gray-300 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5 rounded-xl"
													type="button"
						...
						<div class="flex-1 overflow-y-auto">
							{#if previewNote}
								<div class="bg-white dark:bg-gray-850 rounded-xl p-6 h-full">
									<div class="flex items-center justify-between mb-4">
										<h2 class="text-xl font-semibold">{previewNote.title}</h2>
										<a
									...
									<div class="text-sm text-gray-600 dark:text-gray-400 mb-4">
										{$i18n.t('Updated')} {dayjs(previewNote.updated_at / 1000000).fromNow()}
										{#if previewNote?.user}
											• {$i18n.t('By {{name}}', {
												name: capitalizeFirstLetter(
													previewNote?.user?.name ?? previewNote?.user?.email ?? $i18n.t('Deleted User')
												)
											})}
										{/if}
									</div>
									...
									<div class="prose prose-sm dark:prose-invert max-w-none">
										{#if previewNote.data?.content?.html}
											{@html previewNote.data.content.html}
										{:else if previewNote.data?.content?.md}
											{@html marked.parse(previewNote.data.content.md)}
										{:else}
							...
							{:else}
								<div class="h-full flex items-center justify-center">
									<div class="text-center">
										<div class="text-lg text-gray-400 dark:text-gray-600 mb-2">
											{$i18n.t('Select a note to preview')}
										</div>
										<div class="text-sm text-gray-300 dark:text-gray-700">
											{$i18n.t('Click on any note from the list to see its content')}
										</div>
									</div>
								</div>
							{/if}
