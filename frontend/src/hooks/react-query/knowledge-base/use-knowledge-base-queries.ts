    ...
    queryFn: async (): Promise<KnowledgeBaseListResponse> => {
      ...
      if (!response.ok) {
        ...
        if (response.status === 403) {
          return { entries: [], total_count: 0 } as KnowledgeBaseListResponse;
        }
        const error = await response.text();
...
export function useAgentKnowledgeBaseContext(agentId: string, maxTokens = 4000) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) {
        // Handle 403 gracefully by returning empty context
        if (response.status === 403) {
          return { context: '', token_count: 0 };
        }
        const error = await response.text();
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<ProcessingJobsResponse> => {
      ...
      if (!response.ok) {
        // Handle 403 gracefully by returning empty result
        if (response.status === 403) {
          return { jobs: [], total_count: 0 } as ProcessingJobsResponse;
        }
        const error = await response.text();
